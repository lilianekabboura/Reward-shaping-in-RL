...
######################################### Training Loop #########################################
    def train(self):
        ...
        #log_f_name2 = logging.create_log_files_potentials_details(
        #self.env_name)
        #log_f2 = open(log_f_name2, "w+")
        #log_f2.write("episode,timestep,reward,po_s")
        ...
        for i_episode in range(self.hyperparams.num_episodes):
            ...
            if self.training_method == "epsilon_greedy" or self.training_method == "greedy":
                ...
                score, time_step, total_reward_shaped, steps_done = Methods.run_episode_with_GBRS_AC(self.env_name,
            ...
            if self.training_method == "MixIn":
                ...
                if i_episode % self.hyperparams.log_every == 0 and i_episode > 0:
                    log_f.write('{},{},{},{},{:.2f},{:.2f},{:.2f},{:5.3f},{:5.3f} \n'.format(
                        i_episode, time_step, steps_done, score, avg_score, total_reward_shaped, avg_score_shaped, eps, self.hyperparams.k[i_episode]))
            ...
            if self.training_method == "epsilon_greedy" or self.training_method == "greedy":
                if i_episode % self.hyperparams.print_every == 0 and i_episode > 0:
                    logger.debug('Episode: {:5} Timesteps: {} Score: {:5}  Avg.Score: {:.2f}, eps-greedy: {:5.3f} Time: {:02}:{:02}:{:02}'.format(
                        i_episode, time_step, score, avg_score, eps, dt//3600, dt % 3600//60, dt % 60))
    ...
    def main(self, training_method, policy):
        ...
        if save_model_ == True:
            save_model(save_model_, self.env_name,
                       self.training_method, self.agent, self.curiosity)
        else:
...
def init_hyperparams():
    ...
    hyperparams.hidden_dim = 16
    hyperparams.eps_start = 1
    hyperparams.min_eps = 0.01  # 0.01
    hyperparams.eps_decay = 0.95
    ...
    hyperparams.N = (np.linspace(0.8, 0.3,
                                 hyperparams.num_episodes))
